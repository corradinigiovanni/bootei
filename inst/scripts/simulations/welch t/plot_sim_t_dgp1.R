...
plot_size_power_log10 <- function(k_pick, base_size = 14) {
  ...
  # We set tick length as a fixed FRACTION of the panel width in log10-scale.
  tick_frac      <- 0.085  # <- this makes k=5/7 match your previous "good" length
  lab_gap_log10  <- 0.04   # spacing for the text (in log10-units)
  
  x_min <- min(n_vals)
  x_max <- max(n_vals)
  
  log_range     <- log10(x_max) - log10(x_min)
  tick_w_log10  <- tick_frac * log_range  # <- key fix: scales with panel range (k=3 gets longer)
  ...
  ggplot(df_k,
    ...
    annotate(
      ...
      linewidth = 1.6,   # pronounced tick (as you wanted)
      colour = "black"
    ) +
    annotate(
      "text",
