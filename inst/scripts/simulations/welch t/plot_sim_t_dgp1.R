# ================================================================
# BOOTEI – Welch t-test: analysis of welch_DETAIL_GLOBAL_dgp1.csv
...
ROOT <- "_"
file_global <- file.path(ROOT, "welch_DETAIL_GLOBAL_dgp1.csv")
stopifnot("welch_DETAIL_GLOBAL_dgp1.csv not found!" = file.exists(file_global))
...
plot_power_vs_neff <- function(k_pick) {
  ...
  ggplot(df_k, aes(x = n_eff, y = power, group = method, color = method)) +
    ...
    labs(
      title    = glue("Welch t-test – {k_lab_here} – Power vs n_eff"),
...
plot_size_vs_neff <- function(k_pick) {
  ...
  ggplot(df_k, aes(x = n_eff, y = size, group = method, color = method)) +
    ...
    labs(
      title    = glue("Welch t-test – {k_lab_here} – Type I error vs n_eff (H0)"),
...
make_powersize_panel_welch_dgp1 <- function(k_pick, panel_tag, base_size = 11) {
  ...
  # Fewer x ticks only in panel A if needed
...
# ------------------ Build panels ------------------
powersize_panel_A_welch_dgp1 <- make_powersize_panel_welch_dgp1(
  k_pick = 3L, panel_tag = "A", base_size = 11
)

powersize_panel_B_welch_dgp1 <- make_powersize_panel_welch_dgp1(
  k_pick = 5L, panel_tag = "B", base_size = 11
)

powersize_panel_C_welch_dgp1 <- make_powersize_panel_welch_dgp1(
  k_pick = 7L, panel_tag = "C", base_size = 11
)

# ------------------ Panels row ------------------
panels_row_welch_dgp1 <-
  powersize_panel_A_welch_dgp1 +
  powersize_panel_B_welch_dgp1 +
  powersize_panel_C_welch_dgp1 +
  plot_layout(
    nrow   = 1,
    widths = c(1.28, 1, 1)
  )
...
print(powersize_plot_welch_dgp1)

# ------------------ Save outputs ------------------
ggsave(
  filename = file.path(ROOT, "powersize_plot_welch_dgp1.pdf"),
  plot     = powersize_plot_welch_dgp1,
  width    = 7,
  height   = 6,
  units    = "in",
  device   = cairo_pdf
...
ggsave(
  filename = file.path(ROOT, "powersize_plot_welch_dgp1.tiff"),
  plot     = powersize_plot_welch_dgp1,
  width    = 7,
  height   = 6,
  units    = "in",
