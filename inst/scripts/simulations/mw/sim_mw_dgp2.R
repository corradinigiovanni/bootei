...
## Expects the global results file mw_DETAIL_GLOBAL_dgp2.csv produced by
## inst/scripts/simulations/sim_mw_dgp2.R. Not run during R CMD check.

# ================================================================
# BOOTEI Mann–Whitney (Sobol QMC) – DGP2
# Simulations over (n1, n2) for ordinal outcomes with k = 3, 5, 7 levels
# (k-specific sample-size grids)
#
# - Test: Mann–Whitney (Wilcoxon rank-sum) test, two-sided
#
# - Methods compared:
...
#     * boot_type = "sobol_shift"
#       (Sobol QMC resampling, resampling keys are then fixed and coupled across permutations)
#
# - DGP2 (ordinal Lehmann-type alternative on a latent scale):
#     * Two independent groups of sizes n1 and n2
#     * Latent generation via probability integral transform:
#         U ~ Unif(0,1),  U_theta = U^(1/theta),  Z = F^{-1}(U_theta)
#       with F^{-1} = Φ^{-1} (Normal quantile) in r_lehmann()
#     * Discretisation to Likert categories (common cutpoints):
#         Y = cut(Z; Φ^{-1}(0,1/(k+1),...,1))
#     * H0: theta = 1  (no stochastic dominance; effe = FALSE; assoc = 0)
#     * H1: theta = 1 + assoc_ref(k) * ((n1+n2)/n_min)^(-gamma_assoc)
#         - gamma_assoc = 0.475
#         - n_min = 10 (baseline for scaling)
#         - assoc_ref(k) is k-specific and controls the strength of the departure:
#             k = 3: assoc_ref = 2
#             k = 5: assoc_ref = 3
#             k = 7: assoc_ref = 4
#       (local alternative: effect shrinks with n1+n2, while remaining detectable)
#
# - Design:
#     * Likert levels: k ∈ {3, 5, 7}
#     * Balanced sample sizes (n1 = n2) with k-specific grids:
...
theta_fun <- function(n1, n2,
                      assoc_ref,           # <--- questo argomento DEVE esserci
                      gamma = gamma_assoc,
                      n_min = 10) {
  n_tot <- n1 + n2
...
# QUI usiamo davvero "assoc"
simulate_mw <- function(n1, n2, k, assoc = 0, effe = TRUE) {
  if (!effe || assoc == 0) {
    theta <- 1
  } else {
    theta <- theta_fun(n1, n2, assoc_ref = assoc)  # assoc controlla la forza
  }
  
  x <- simulate_group_likert_lehmann(n1, k, theta = 1)      # controllo
  y <- simulate_group_likert_lehmann(n2, k, theta = theta)  # trattamento
...
for (k_i in k_list) {
  ...
  for (row_idx in seq_len(nrow(mw_pairs_k))) {
    ...
    # H1: adesso assoc è solo il "ref" per quel k
    ...
    res_H1 <- run_one_pair_batch_mw(
      ...
      n2               = n2_cur,
      alpha            = alpha_test,
      assoc            = assoc_H1_cur,  # passa il ref
      effe             = TRUE,
      sim_MC_per_job   = sim_MC_per_job,
      n_rep            = n_rep,
      B_boot           = B_boot,
...
if (nrow(detail_global) > 0L) {
  write_csv(detail_global, file.path(ROOT, "mw_DETAIL_GLOBAL_dgp2.csv"))
}
